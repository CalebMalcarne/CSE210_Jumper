...
class Director:
    ...
    def _do_outputs(self):
        ...
        Display.split_word(Display, word.get_Word(word), self.answers)
    ...
    def _get_inputs(self):
        ...
        while self.user_input.lower() not in possible_answers:
            self.user_input = input("Oops!\nPlease guess a letter [a-z]: ")
    ...
    def _get_updates(self):
        ...
        change = comparison.compare(word._chosen_Word, self.user_input)
        if change == True:
            self.answers.append(self.user_input)
            if ''.join(self.answers) == word.get_Word(word):
                self._is_playing = False
                
        else:
            Display.update_jumper(Display)

            if Display.get_Status(Display) == 3:
                Display.lose(Display)
                self._is_playing = False
